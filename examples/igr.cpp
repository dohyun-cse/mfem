...
//
// Description:  This example code solves the SWE system of
...
int main(int argc, char *argv[])
{
   ...
   if (visualization)
   {
      ...
      sout_sigma.open(vishost, visport);
      if (!sout_sigma)
      {
         visualization = false;
         if (Mpi::Root())
      ...
      {
         sout_sigma.precision(precision);
         sout_sigma << "parallel " << numProcs << " " << myRank << "\n";
         sout_sigma << "solution\n" << pmesh << sigma;
         sout_sigma << "window_title 'sigma, t = 0'\n";
         sout_sigma << "view 0 0\n";  // view from top
         sout_sigma << "keys jlm\n";  // turn off perspective and light, show mesh
         sout_sigma << flush;
         MPI_Barrier(pmesh.GetComm());
   ...
   for (int ti = 0; !done;)
   {
      ...
      if (done || ti % vis_steps == 0)
      {
         ...
         if (visualization)
         {
            ...
            sout << "parallel " << numProcs << " " << myRank << "\n";
            sout << "solution\n" << pmesh << height << flush;
            sout_sigma << "window_title 'sigma, t = " << t << "'\n";
            sout_sigma << "parallel " << numProcs << " " << myRank << "\n";
            sout_sigma << "solution\n" << pmesh << sigma << flush;
         }
      }
