...
void SundialsNVector::_SetNvecDataAndSize_(long glob_size)
{
   ...
   switch (id)
   {
      ...
      case SUN_HIP_OR_CUDA(SUNDIALS_NVEC):
      {
         ...
         static_cast<SUN_Hip_OR_Cuda(N_VectorContent)>(GET_CONTENT(local_x))->length = size;
         break;
      }
#endif
#ifdef MFEM_USE_MPI
...
N_Vector SundialsNVector::MakeNVector(bool use_device)
{
   ...
   if (use_device)
   {
      x = SUN_Hip_OR_Cuda(N_VNewWithMemHelp)(0, UseManagedMemory(), Sundials::GetMemHelper(),
                                 Sundials::GetContext());
   }
   else
   {
...
N_Vector SundialsNVector::MakeNVector(MPI_Comm comm, bool use_device)
{
   ...
   else
   {
      ...
      if (use_device)
      {
         x = N_VMake_MPIPlusX(comm, SUN_Hip_OR_Cuda(N_VNewWithMemHelp)(0, UseManagedMemory(),
