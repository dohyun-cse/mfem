...
public:
   ...
   virtual real_t conjugate(const real_t x) const
   { MFEM_ABORT("LegendreFunction::conjugate not implemented."); }
   virtual real_t conjugate(const real_t x, ElementTransformation &Tr) const
   { return conjugate(x); }
   virtual real_t conjugate(const Vector &x) const
   {
      real_t sum = 0.0;
      for (auto val : x) { sum += this->conjugate(val); }
      return sum;
   }
   virtual real_t conjugate(const Vector &x, ElementTransformation &Tr) const
   {
      real_t sum = 0.0;
      for (auto val : x) { sum += this->conjugate(val, Tr); }
      return sum;
   }
...
public:
   ...
   using LegendreFunction::operator();
   real_t operator()(const Vector &x) const override;
   using LegendreFunction::conjugate;
   real_t conjugate(const Vector &x) const override;
   using LegendreFunction::grad;
   void grad(const Vector &x, Vector &g) const override;
   using LegendreFunction::gradinv;
   void gradinv(const Vector &x, Vector &invg) const override;
...
class PolytopeGibbs : public LegendreFunction
{
   real_t tol;
   Gibbs gibbs;
   ...
   DenseMatrix V;
   mutable Vector Vtx;
   mutable Vector lambda;
   mutable DenseMatrix Hg;
   mutable DenseMatrix VHg;
public:
   PolytopeGibbs(const DenseMatrix &V_, real_t tol=1e-10)
      : tol(tol), gibbs(tol)
      , V(V_), Vtx(V_.Width()), lambda(V_.Width())
      , Hg(V_.Width()), VHg(V_.Height(), V_.Width())
   {}
   using LegendreFunction::operator();
   real_t operator()(const Vector &x) const override
   {
      MFEM_ABORT("NOT IMPLEMENTED");
      // requires solving a convex optimization
      // There is no short term plan to implement this
   }
   using LegendreFunction::conjugate;
   real_t conjugate(const Vector &x) const override
   {
      V.MultTranspose(x, Vtx);
      return gibbs.conjugate(Vtx);
   }
   ...
   {
      V.MultTranspose(x, Vtx);
      gibbs.hessinv(Vtx, Hg);
      Mult(V, Hg, VHg);
      MultABt(VHg, V, H);
   }
   void GetBarycenter(const Vector &x, Vector &g) const
   {
      V.MultTranspose(x, Vtx);
      gibbs.gradinv(Vtx, g);
   }
...
class PrimalJacobianCoefficient : public Coefficient
{
   ...
   bool own_coeff;
public:
   PrimalJacobianCoefficient(GridFunction &psi, const LegendreFunction &func)
      : f(func), latent(new GridFunctionCoefficient(&psi)), own_coeff(true) {}
...
public:
   ...
   void Eval(DenseMatrix &hessR, ElementTransformation &Tr,
             const IntegrationPoint &ip) override
   {
      MFEM_ASSERT(latent != nullptr, "Latent VectorCoefficient is null.");
      hessR.SetSize(GetHeight());
      latent->Eval(val, Tr, ip);
      f.hessinv(val, Tr, hessR);
   }
};
